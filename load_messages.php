...
while ($row = $result->fetch_assoc()) {
    ...
    $class = ($row['sender_id'] == $current_user_id)
        ? "outgoing"
        : "incoming";
...
if (!empty($claim['proof_image'])) {
    ...
    echo '
    <img 
        src="'.htmlspecialchars($claim['proof_image']).'" 
        class="claim-proof"
    >';
}
echo '
...
if (($item_owner['user_id'] ?? null) == $current_user_id) {
    ...
    if ($isFinal) {
        echo '<div class="claim-actions">
                <small style="color:gray;">Decision already made</small>
              </div>';
    } else {
...
<div class="claim-actions">
    ...
    <form method="POST" action="approve_claim.php"
        onsubmit="return confirm(\'Are you sure you want to APPROVE this claim? This cannot be undone.\');">
    ...
    <form method="POST" action="reject_claim.php"
        onsubmit="return confirm(\'Are you sure you want to REJECT this claim? This cannot be undone.\');">
...
';
    ...
    <div class="claim-card-wrapper '.$class.'">
        ...
        <div class="claim-card">
            ...
            <div class="claim-info">
                <strong>Name:</strong>
                '.htmlspecialchars($report['finder_name']).'
            </div>

            <div class="claim-info">
                <strong>Contact:</strong>
                '.htmlspecialchars($report['finder_contact']).'
            </div>
    ...
    if (!empty($report['proof_image'])) {
        ...
        echo '
        <img
            src="'.htmlspecialchars($report['proof_image']).'"
            class="claim-proof"
        >';
    }
